...
contract PurchaseRecord is PurchaseRecordInterface, Ownable {
    ...
    mapping(address => mapping(uint256 => Record)) escrowToRecords;
    ...
    function confirmDeposit(address _address) external onlyEscrow {
        addRecord(_address, State.Paid, msg.sender);
    }

    function sendItem(address _address) external onlyEscrow {
        addRecord(_address, State.Sent, msg.sender);
    }

    function cancelPurchase(address _address) external onlyEscrow {
        addRecord(_address, State.Cancelled, msg.sender);
        addRecord(_address, State.End, address(this));
    }

    function refund(address _address) external onlyEscrow {
        addRecord(_address, State.Refunded, msg.sender);
        addRecord(_address, State.End, address(this));
    }

    function confirmPurchase(address _address) external onlyEscrow {
        addRecord(_address, State.Complete, msg.sender);
        addRecord(_address, State.End, address(this));
    }

    function getRecordNumber(address _address)
        public
        view
    ...
    function getRecord(address _address, uint256 recordNo)
        public
        view
        returns (
            State _state,
